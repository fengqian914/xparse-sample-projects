...
const STEPS = ['上传文件', 'OCR 解析', '文档分类', '字段抽取', '查看结果']

function getStepIndex(extractionPhase) {
  if (extractionPhase === 'idle') return 1        // OCR done, not yet classifying
  if (extractionPhase === 'classifying') return 2
  if (extractionPhase === 'extracting') return 3
  return 4                                         // done or error
}
